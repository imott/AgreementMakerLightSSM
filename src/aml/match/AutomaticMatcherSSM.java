...
public class AutomaticMatcherSSM {
	...
	private static double selThresh;
	private static double lowerThresh = 0.4;
	...
	private static Alignment aLowerThreshold;
	private static Alignment lex;
	// Semantic Similarity parameters
	private static SemanticSimilarity semanticMeasure;
	private static SemanticSimilarity IC;
	private static SemanticSimilarity weighting;
	private static SemanticSimilarity finalScore;
	...
	public static void match() throws UnsupportedEntityTypeException {
		...
		a = new Alignment();
		lowerThresh = aml.getThreshold();
		selThresh = aml.getThreshold();
	...
	public static void match(double threshold, double selectorThreshold) throws UnsupportedEntityTypeException {
		// Get the AML instance
		aml = AML.getInstance();
		// source = aml.getSource();
		// target = aml.getTarget();
		...
		// Initialize the alignment
		a = new Alignment();
		aLowerThreshold = new Alignment();
		thresh = threshold;

		selThresh = selectorThreshold;
		if (matchClasses) {
	...
	public static void match(double threshold, double selectorThreshold, SemanticSimilarity... args)
		...
		if (args.length >= 1)
			IC = args[0];
		match(threshold, selectorThreshold);
	}

	public static void match(double selectorThreshold, double threshold, int rad, SemanticSimilarity... args)
			throws UnsupportedEntityTypeException {
		radius = rad;
		if (args.length == 5)
			finalScore = args[4];
	...
	public static void match(double lowerThreshold, double selectorThreshold, double threshold, int rad,
			SemanticSimilarity... args) throws UnsupportedEntityTypeException {
	...
	private static void matchClasses() throws UnsupportedEntityTypeException {
		...
		Alignment lexLower = lm.match(EntityType.CLASS, lowerThresh);
		a.addAll(lex);
		System.out.println(lexLower.size());
		aLowerThreshold.addAll(lexLower);
		...
		if (lang.equals(LanguageSetting.SINGLE)) {
			if (size.equals(SizeCategory.SMALL)) {
				...
				// Deciding whether to use it based on its coverage of the input
				// ontologies
				// (as we expect a high gain if the coverage is high given that
				// WordNet will
				...
				for (String bk : bkSources) {
					// In the case of BK Lexicons and Ontologies, we decide
					// whether to use them
					// based on their mapping gain (over the direct Lexical
					// alignment)
					if (bk.endsWith(".lexicon")) {
						...
						// In the case of Ontologies, if the mapping gain is
						// very high, we can
						// use them for Lexical Extension, which will
						// effectively enable Word-
						...
						if (gain >= HIGH_GAIN_THRESH) {
							...
							xr.extendLexicons();
							// If that is the case, we must compute a new
							// Lexical alignment
							// after the extension
		...
		if (!size.equals(SizeCategory.HUGE)) {
			...
			Alignment wordLower = new Alignment();
			...
			if (lang.equals(LanguageSetting.SINGLE)) {
				...
				wordLower.addAll(wm.match(EntityType.CLASS, lowerThresh));
			...
			aLowerThreshold.addAllOneToOne(wordLower);
		}
		StringMatcherNoWeight psm = new StringMatcherNoWeight();
		// If the task is small, we can use the PSM in match mode
		if (size.equals(SizeCategory.SMALL)) {
			if (lang.equals(LanguageSetting.SINGLE)) {
				a.addAll(psm.match(EntityType.CLASS, thresh));

				aLowerThreshold.addAll(psm.match(EntityType.CLASS, lowerThresh));
		...
		// Otherwise we use it in extendAlignment mode
			...
			aLowerThreshold.addAllOneToOne(psm.extendAlignment(a, EntityType.CLASS, lowerThresh));
		}
		if (!size.equals(SizeCategory.HUGE)) {
			SpacelessLexicalMatcher sl = new SpacelessLexicalMatcher();

			a.addAllNonConflicting(sl.match(EntityType.CLASS, thresh));
			aLowerThreshold.addAllNonConflicting(sl.match(EntityType.CLASS, lowerThresh));
			double nameRatio = Math.max(
					1.0 * source.getLexicon().nameCount(EntityType.CLASS) / source.count(EntityType.CLASS),
		...
		if (size.equals(SizeCategory.SMALL) || size.equals(SizeCategory.MEDIUM)) {
			NeighborSimilarityMatcher nsm = new NeighborSimilarityMatcher(aml.getNeighborSimilarityStrategy(),
					aml.directNeighbors());
		...
		if (size.equals(SizeCategory.HUGE)) {
			...
			NeighborSimilarityMatcher nb = new NeighborSimilarityMatcher(NeighborSimilarityStrategy.MAXIMUM, true);
			Alignment c = nb.rematch(a, EntityType.CLASS);
			b = LWC.combine(b, c, 0.75);
			b = LWC.combine(a, b, 0.8);
			Selector s = new Selector(selThresh - 0.05, sType);
			b = s.filter(b);
			s = new Selector(selThresh, sType, b);
			s.filter();
		} else if (!im.isInteractive()) {
			Selector s = new Selector(selThresh, sType);
			s.filter();
		}
		if (im.isInteractive()) {
			if (size.equals(SizeCategory.SMALL))
		...
		Alignment aToExtend = aml.getAlignment();

		SemanticEqMatcher ssm = new SemanticEqMatcher(aLowerThreshold, radius, IC, semanticMeasure, strat, weighting,
				finalScore);

		Alignment afterSSM = ssm.extendAlignment(aToExtend, EntityType.CLASS, thresh);

		aToExtend.addAll(afterSSM);

		aml.setAlignment(aToExtend);
	...
	private static void matchIndividuals() throws UnsupportedEntityTypeException {
		...
		// Process matching problem
			...
			double cov = Math.min(b.sourceCoverage(EntityType.INDIVIDUAL), b.targetCoverage(EntityType.INDIVIDUAL));
